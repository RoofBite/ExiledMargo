...
43: {"texture": "stairs_down.png", "scale_x": 1.0, "scale_y": 1.0, "blocking": True, "z": 0,
    ...
    3: {"texture": "rat.png", "scale_x": 0.5, "scale_y": 0.5, "blocking": True, "z": 0,
        "type": "monster", "name": "Szczur", "hp": 15, "attack": 4, "defense": 0, "xp_yield": 25,
        "loot_table": {
    ...
    14: {"texture": "jelen02.png", "scale_x": 1.2, "scale_y": 0.8, "blocking": True, "z": 0,
         ...
         "loot_table": {
            frozenset({"name": "Mięso", "value": 5, "type": "consumable", "heals": 10}.items()): 0.5, 
            frozenset({"name": "Skóra Jelenia", "value": 12, "type": "loot"}.items()): 0.3 
        }},
        
        15: {"texture": "bear.png", "scale_x": 0.7, "scale_y": 0.7, "blocking": True, "z": 0,
         "type": "monster", "name": "Niedzwiedz", "hp": 50, "attack": 12, "defense": 2, "xp_yield": 25, 
         
         "loot_table": {
            frozenset({"name": "Mięso", "value": 5, "type": "consumable", "heals": 10}.items()): 0.5, 
            frozenset({"name": "Skóra Niedzwiedzia", "value": 20, "type": "loot"}.items()): 0.3 
        }},
    ...
    22: {"texture": "rat_king.png", "scale_x": 0.5, "scale_y": 0.5, "blocking": True, "z": 0, "aggressive": True,
     ...
     23: {"texture": "barell.png", "scale_x": 0.5, "scale_y": 0.5, "blocking": True, "z": 0, "aggressive": False,
        "type": "monster", "name": "Beczka", "hp": 50, "attack": 1, "defense": 3, "xp_yield": 10,
        "loot_table": {
            frozenset({"name": "Mikstura leczenia", "value": 25, "type": "consumable", "heals": 50}.items()): 1.0
        }},
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                ...
                if leave_rect.collidepoint(mx, my): game_state.end_dialogue()
                    ...
                    if 'accept_quest' in action_rects and action_rects['accept_quest'].collidepoint(mx, my):
                        ...
                        player.add_xp(quest.reward['xp'], game_state); player.money += quest.reward['money']; quest.is_turned_in = True; del player.active_quests[quest.name]
    ...
    for item_frozenset, chance in monster.loot_table.items():
        if random.random() < chance:
            if len(player.inventory) < player.inventory_limit:
                ...
                game_state.set_info_message(f"Zdobyto: {item_dict['name']} (Wartość: {item_dict['value']} zł)", 4000)
            else:
