...
class Renderer:
    def __init__(self, screen, player, maps, walls, sprites):
        ...
        # Konwertujemy tekstury do tablic numpy dla superszybkiego dostępu
        try:
            floor_tex_raw = pygame.image.load(os.path.join(TEXTURE_PATH, "grass.png")).convert()
            self.floor_texture_np = pygame.surfarray.array3d(floor_tex_raw)
        except pygame.error:
            floor_tex_raw = pygame.Surface((TILE, TILE)); floor_tex_raw.fill((100, 80, 50))
            self.floor_texture_np = pygame.surfarray.array3d(floor_tex_raw)

        self.CEILING_TEXTURES_NP = {}
        for floor, fname in CEILING_TEXTURE_BY_FLOOR.items():
            try:
                ceil_tex_raw = pygame.image.load(os.path.join(TEXTURE_PATH, fname)).convert()
                self.CEILING_TEXTURES_NP[floor] = pygame.surfarray.array3d(ceil_tex_raw)
            except pygame.error:
                ceil_tex_raw = pygame.Surface((TILE, TILE)); ceil_tex_raw.fill((50, 150, 250))
                self.CEILING_TEXTURES_NP[floor] = pygame.surfarray.array3d(ceil_tex_raw)
                
        # Bufor pikseli ekranu jako tablica numpy
        self.pixel_buffer = np.zeros((RENDER_WIDTH, RENDER_HEIGHT, 3), dtype=np.uint8)

    def draw_floor_and_ceiling(self):
        """Implementacja wydajnego rysowania podłogi i sufitu na podstawie tutoriala Lode'a Vandevenne."""
        ...
        floor_tex = self.floor_texture_np
        ceil_tex = self.CEILING_TEXTURES_NP.get(self.player.floor, self.CEILING_TEXTURES_NP.get(0))
        tex_height, tex_width, _ = floor_tex.shape # Uwaga: numpy zwraca (wys, szer)

        render_half_height = RENDER_HEIGHT // 2
        horizon_y_abs = render_half_height + int(self.player.pitch * RENDER_SCALE)

        for y in range(RENDER_HEIGHT):
            is_floor = y > horizon_y_abs
            ...
            if is_floor:
                colors = floor_tex[tex_coords_x, tex_coords_y]
            else:
                colors = ceil_tex[tex_coords_x, tex_coords_y]

            self.pixel_buffer[:, y] = colors

        # POPRAWKA: Usunięto .transpose(1, 0, 2). Kopiujemy bufor bezpośrednio.
