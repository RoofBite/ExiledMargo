...
SPRITE_PROPERTIES = {
    ...
    20: {"texture": "merchant.png", "scale_x": 1.0, "scale_y": 1.0, "blocking": True, "z": 0,
         ...
         "sells": [
             {"name": "Solidny miecz", "value": 90, "type": "weapon", "attack": 5*2},
             {"name": "Średni miecz", "value": 40, "type": "weapon", "attack": 3*2},
             {"name": "Skórzana zbroja", "value": 130, "type": "armor", "defense": 3*2},
             {"name": "Żelazny hełm", "value": 80, "type": "helmet", "defense": 2*2},
             {"name": "Mikstura leczenia", "value": 25, "type": "consumable", "heals": 50*2}
         ]},
    21: {"texture": "healer.png", "scale_x": 1.0, "scale_y": 1.0, "blocking": True, "z": 0,
     ...
     "quest": Quest(
         ...
         reward={'xp': 120, 'money': 10, 'items': [
             {"name": "Uszkodzona tarcza", "value": 3, "type": "shield", "defense": 1*2}
         ]}
     )},
...
class Player:
    ...
    def grid_move(self, forward, sprites, game_state, logger,player):
        ...
        if tile_content:
            for block in tile_content:
                is_blocking_wall = (
                    ...
                    block.get('z', 0) == 0 and 'target' not in block
                )
                if is_blocking_wall:
                    return
        ...
        if tile_content and 'target' in tile_content[0]:
            self.change_floor(tile_content[0]['target'], game_state,logger)
