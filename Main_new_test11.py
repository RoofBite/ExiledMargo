...
WALL_PROPERTIES = {
    ...
    96: {"transparent": False}, # Drzwi na razie traktujemy jako nieprzezroczyste
    95: {"transparent": True, "texture_id": 1} # ID 95 to nasz nowy płot/krata
}
...
43: {"texture": "stairs_down.png", "scale_x": 1.0, "scale_y": 1.0, "blocking": True, "z": 0,
    ...
    98: {
    "texture": "wall5.png", "scale_x": 1.5, "scale_y": 1, 
...
def game_loop_step(player, game_state, renderer, sprites, screen, render_surface, clock, font, ui_font, info_font, sprite_properties, sprite_textures):
    ...
    renderer.z_buffer.fill(float('inf'))
...
async def main():
    ...
    # NOWA, INTELIGENTNA PĘTLA WCZYTUJĄCA SPRATY
    sprites = []
            ...
            for rx, vals in enumerate(row):
                ...
                for v in vals:
                    ...
                    if sprite_id in SPRITE_PROPERTIES:
                        # --- LOGIKA TWORZENIA "KRZYŻAKA" ---
                        # Jeśli to jeden z naszych sprajtów-ścian
                        if sprite_id in [98, 99]:
                            # Stwórz oba sprajty: pionowy (98) i poziomy (99)
                            # i umieść je w tym samym miejscu, na środku pola.
                            
                            # Sprajt pionowy
                            props_98 = SPRITE_PROPERTIES[98].copy()
                            tex_98 = SPRITE_TX[98]
                            sprites.append(Sprite((rx + 0.5) * TILE, (ry + 0.5) * TILE, fl, props_98, tex_98, 98))

                            # Sprajt poziomy
                            props_99 = SPRITE_PROPERTIES[99].copy()
                            tex_99 = SPRITE_TX[99]
                            sprites.append(Sprite((rx + 0.5) * TILE, (ry + 0.5) * TILE, fl, props_99, tex_99, 99))

                            # Ponieważ dodaliśmy oba, nie musimy już przetwarzać oryginalnego `v`.
                            # Używamy `break`, aby wyjść z pętli `for v in vals`.
                            break 
                        else:
                            # Dla wszystkich innych sprajtów, działaj normalnie
...
class Renderer:
    def __init__(self, screen, player, maps, walls, sprites):
        ...
        self.z_buffer = np.full((RENDER_WIDTH, RENDER_HEIGHT), float('inf'))
    ...
    def draw_sprites(self):
        # Sortowanie sprajtów od najdalszego do najbliższego
        sorted_sprites = sorted(
            [s for s in self.sprites if s.floor == self.player.floor and not s.is_dead],
            key=lambda s: math.hypot(s.x - self.player.x, s.y - self.player.y),
            reverse=True
        )

        for sprite in sorted_sprites:
            # Transformacja pozycji sprajta względem kamery
            rel_x, rel_y = sprite.x - self.player.x, sprite.y - self.player.y
            inv_det = 1.0 / (self.player.planeX * self.player.dirY - self.player.dirX * self.player.planeY)
            
            transform_x = inv_det * (self.player.dirY * rel_x - self.player.dirX * rel_y)
            transform_y = inv_det * (-self.player.planeY * rel_x + self.player.planeX * rel_y)

            # Nie rysuj sprajtów za graczem
            if transform_y <= 0.1: continue

            # Obliczenia projekcji na ekranie
            sprite_screen_x = int((RENDER_WIDTH / 2) * (1 + transform_x / transform_y))
            proj_height = abs(int(RENDER_HEIGHT / transform_y)) * sprite.scale_y
            sprite_width = abs(int(RENDER_HEIGHT / transform_y)) * sprite.scale_x

            draw_start_y = max(0, int(RENDER_HEIGHT / 2 - proj_height / 2 + self.player.pitch * RENDER_SCALE))
            draw_end_y = min(RENDER_HEIGHT - 1, int(RENDER_HEIGHT / 2 + proj_height / 2 + self.player.pitch * RENDER_SCALE))
            draw_start_x = max(0, int(sprite_screen_x - sprite_width / 2))
            draw_end_x = min(RENDER_WIDTH - 1, int(sprite_screen_x + sprite_width / 2))
            
            tex = sprite.texture
            tex_w, tex_h = tex.get_size()

            for x in range(draw_start_x, draw_end_x):
                # Sprawdź z buforem Z czy sprajt nie jest za ścianą
                if self.z_buffer[x, 0] > transform_y: 
                    tex_x = int((x - (sprite_screen_x - sprite_width / 2)) * tex_w / sprite_width)
                    if 0 <= tex_x < tex_w:
                        for y in range(draw_start_y, draw_end_y):
                             # Sprawdź z buforem Z dla każdego piksela
                            if self.z_buffer[x, y] > transform_y:
                                tex_y = int(((y - (RENDER_HEIGHT/2 - proj_height/2 + self.player.pitch * RENDER_SCALE)) / proj_height) * tex_h)
                                if 0 <= tex_y < tex_h:
                                    color = tex.get_at((tex_x, tex_y))
                                    if color.a == 255: # Rysuj tylko nieprzezroczyste piksele
                                        self.screen.set_at((x, y), color)
                                        # Aktualizuj Z-Buffer
                                        self.z_buffer[x, y] = transform_y
    ...
    def draw_walls(self):
        ...
        for x in range(RENDER_WIDTH):
            ray_angle = self.player.angle - HALF_FOV + (x / RENDER_WIDTH) * FOV
            sin_a, cos_a = math.sin(ray_angle), math.cos(ray_angle)
            
            # DDA Setup
            map_x, map_y = int(self.player.x / TILE), int(self.player.y / TILE)
            delta_dist_x = abs(1 / cos_a) if cos_a != 0 else float('inf')
            delta_dist_y = abs(1 / sin_a) if sin_a != 0 else float('inf')

            if cos_a < 0:
                step_x = -1
                side_dist_x = (self.player.x / TILE - map_x) * delta_dist_x
            else:
                step_x = 1
                side_dist_x = (map_x + 1.0 - self.player.x / TILE) * delta_dist_x

            if sin_a < 0:
                step_y = -1
                side_dist_y = (self.player.y / TILE - map_y) * delta_dist_y
            else:
                step_y = 1
                side_dist_y = (map_y + 1.0 - self.player.y / TILE) * delta_dist_y
            ...
            for _ in range(MAX_DEPTH * 2):
                ...
                if not (0 <= map_y < len(current_map) and 0 <= map_x < len(current_map[0])):
                    break

                blocks = current_map[map_y][map_x]
                if not blocks: continue

                wall_id = blocks[0].get('id')
                if wall_id not in self.WALL_PROPERTIES: continue
                
                wall_props = self.WALL_PROPERTIES[wall_id]
                is_transparent = wall_props.get("transparent", False)
                
                if side == 0:
                    depth = (map_x - self.player.x / TILE + (1 - step_x) / 2) / cos_a
                else:
                    depth = (map_y - self.player.y / TILE + (1 - step_y) / 2) / sin_a
                
                # Rysowanie ściany (zarówno solidnej jak i przezroczystej)
                proj_height = PROJ_COEFF / (depth + 1e-6)
                draw_start = max(0, int(RENDER_HEIGHT / 2 - proj_height / 2 + self.player.pitch * RENDER_SCALE))
                draw_end = min(RENDER_HEIGHT - 1, int(RENDER_HEIGHT / 2 + proj_height / 2 + self.player.pitch * RENDER_SCALE))

                if side == 0:
                    wall_x = self.player.y / TILE + depth * sin_a
                else:
                    wall_x = self.player.x / TILE + depth * cos_a
                wall_x -= math.floor(wall_x)

                tex_id = wall_props.get("texture_id", wall_id)
                ...
                # Rysowanie piksel po pikselu z testem Z-Buffera
                for y in range(draw_start, draw_end):
                    if self.z_buffer[x, y] > depth:
                        tex_y = int(((y - (RENDER_HEIGHT / 2 - proj_height / 2 + self.player.pitch * RENDER_SCALE)) / proj_height) * tex_h)
                        color = tex.get_at((tex_x, tex_y))
                        # Rysuj tylko nieprzezroczyste piksele
                        if color.a == 255: 
                            if side == 1: # Cieniowanie
                                r, g, b, _ = color
                                color = (r//2, g//2, b//2)
                            self.screen.set_at((x, y), color)
                            self.z_buffer[x, y] = depth
                
                # Jeśli ściana jest solidna, kończymy promień
