...
class SnowParticle:
    ...
    def update_gravity(self):
        ...
        sway = self.sway_amplitude * math.sin(self.y * self.sway_frequency + self.sway_offset)
        self.x += sway
...
class SnowManager:
    def __init__(self, num_particles, screen_width, screen_height):
        self.particles = [SnowParticle(screen_width, screen_height) for _ in range(num_particles)]
...
class WeatherManager:
    def __init__(self, rain_manager, snow_manager, maps, wall_ids, plane_properties):
        ...
        self.transitions = {
            ...
            "Pochmurno":  {"Słonecznie": 0.30, "Pochmurno": 0.40, "Deszczowo": 0.15, "Śnieg": 0.15},
        ...
        initial_weights = [0.40, 0.40, 0.1, 0.1] # Zmniejszono szansę na start w deszczu/śniegu
        self.current_state = random.choices(initial_states, weights=initial_weights, k=1)[0]
        
        if self.current_state == "Deszczowo":
            self.is_raining = True
            self.weather_duration_timer = pygame.time.get_ticks() + random.randint(self.WEATHER_MIN_DURATION, self.WEATHER_MAX_DURATION)
    ...
    def trigger_new_weather_period(self):
        ...
        print(f"INFO: Stan pogody zmienił się z '{previous_state}' na: '{self.current_state}'")
    ...
    def update(self, player):
        if (self.is_raining or self.is_snowing) and pygame.time.get_ticks() > self.weather_duration_timer:
        ...
        if current_map:
            ...
            if 0 <= map_y < len(current_map) and 0 <= map_x < len(current_map[0]):
                for block in current_map[map_y][map_x]:
                    ...
                    if block_id in self.plane_properties and self.plane_properties[block_id].get('type') == 'ceiling':
                        is_sheltered = True
                        break
...
#         "type": "portal", "is_portal": True},
    ...
    22: {"texture": "rat_king2.png", "scale_x": 0.5, "scale_y": 0.5, "blocking": True, "z": 0, "aggressive": True,
        "type": "monster", "name": "Król Szczurów", "hp": 100*1.8, "attack": 12*2, "defense": 5*2, "xp_yield": 450*2,
        "loot_table": [
