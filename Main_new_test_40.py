...
class Renderer:
    ...
    def draw_sprites(self):
        ...
        for spr, dist_corr, gamma, theta in sprites_with_depth:
            ...
            # W klasie Renderer - wklej tę funkcję w miejsce starej draw_walls
    def draw_walls(self):
        ...
        # --- PRZYGOTOWANIE TEKSTUR I ZMIENNYCH ---
        level_defaults = LEVEL_TEXTURES.get(self.player.floor, {})
        default_floor_tex_name = level_defaults.get("floor")
        default_ceil_tex_name = level_defaults.get("ceiling")
        
        default_floor_texture = self.HORIZONTAL_SURFACES.get(default_floor_tex_name) if default_floor_tex_name else None
        
        default_ceil_texture = None
        if isinstance(default_ceil_tex_name, str):
            default_ceil_texture = self.HORIZONTAL_SURFACES.get(default_ceil_tex_name)
    
        pos_x, pos_y = self.player.x, self.player.y
        horizon = RENDER_HEIGHT // 2 + int(self.player.pitch * RENDER_SCALE)
        
        # --- GŁÓWNA PĘTLA RAYCASTINGU ---
        for ray in range(NUM_RAYS):
            sin_a, cos_a = math.sin(cur_angle), math.cos(cur_angle)
            
            # --- KROK 1: ZNAJDOWANIE ŚCIAN (DDA) ---
            ...
            for _ in range(MAX_DEPTH):
                # <<< KLUCZOWA ZMIANA ZNAJDUJE SIĘ TUTAJ >>>
                # Obliczamy wx od razu, aby uniknąć błędów
                if side_dx < side_dy:
                    depth = side_dx
                    side_dx += delta_x
                    x_map += step_x
                    # Trafiliśmy w pionową ścianę, więc koordynat tekstury zależy od 'y'
                    wx = pos_y + depth * sin_a
                else:
                    depth = side_dy
                    side_dy += delta_y
                    y_map += step_y
                    # Trafiliśmy w poziomą ścianę, więc koordynat tekstury zależy od 'x'
            ...
            # --- KROK 2: RYSOWANIE PIONOWEJ KOLUMNY (BEZ ZMIAN W TEJ CZĘŚCI) ---
            column_segments.sort(key=lambda x: x[0], reverse=True)
            
            x1 = int(ray * column_width)
            w = max(1, int((ray + 1) * column_width) - x1)
            
            floor_start_y = RENDER_HEIGHT
            ceil_end_y = 0
    
            for _, y_top, y_bottom, tex_id, off, _, _ in column_segments:
                start_pixel_floor = max(int(y_bottom), horizon)
                for y in range(start_pixel_floor, floor_start_y):
                    dist = (self.player.height_in_level * DIST) / (y - horizon + 1e-6)
                    world_x = pos_x + dist * cos_a
                    world_y = pos_y + dist * sin_a
                    map_x, map_y = int(world_x // TILE), int(world_y // TILE)
                    texture_to_use = default_floor_texture
                    custom_surface_data = self.custom_surfaces.get(self.player.floor, {}).get((map_x, map_y))
                    if custom_surface_data and custom_surface_data.get('floor'):
                        custom_tex_name = custom_surface_data['floor'].get('texture')
                        custom_texture = self.HORIZONTAL_SURFACES.get(custom_tex_name)
                        if custom_texture:
                            texture_to_use = custom_texture
                    if texture_to_use:
                        ...
                        pygame.draw.line(self.screen, color, (x1, y), (x1 + w - 1, y))
    
                end_pixel_ceil = min(int(y_top), horizon)
                texture_to_use_ceil = default_ceil_texture
                ceil_color_to_use = default_ceil_tex_name if isinstance(default_ceil_tex_name, tuple) else None
    
                for y in range(ceil_end_y, end_pixel_ceil):
                    dist = ((TILE - self.player.height_in_level) * DIST) / (horizon - y + 1e-6)
                    world_x = pos_x + dist * cos_a
                    world_y = pos_y + dist * sin_a
                    map_x, map_y = int(world_x // TILE), int(world_y // TILE)
                    custom_surface_data = self.custom_surfaces.get(self.player.floor, {}).get((map_x, map_y))
                    if custom_surface_data and custom_surface_data.get('ceiling'):
                        custom_tex_name = custom_surface_data['ceiling'].get('texture')
                        custom_texture = self.HORIZONTAL_SURFACES.get(custom_tex_name)
                        if custom_texture:
                            texture_to_use_ceil = custom_texture
                            ceil_color_to_use = None
                    if texture_to_use_ceil:
                        tex_w, tex_h = texture_to_use_ceil.get_size()
                        tex_x = int(world_x / TILE * tex_w) % tex_w
                        tex_y = int(world_y / TILE * tex_h) % tex_h
                        color = texture_to_use_ceil.get_at((tex_x, tex_y))
                        pygame.draw.line(self.screen, color, (x1, y), (x1 + w - 1, y))
                    elif ceil_color_to_use:
                        pygame.draw.line(self.screen, ceil_color_to_use, (x1, y), (x1 + w - 1, y))
    
                if y_bottom > y_top:
                    target_h = int(y_bottom - y_top)
                    cache_key = (tex_id, off, target_h, w); col = self.texture_cache.get(cache_key)
                    if col is None:
                ...
                floor_start_y = min(floor_start_y, int(y_top))
                ceil_end_y = max(ceil_end_y, int(y_bottom))
                
            if floor_start_y > horizon:
                for y in range(horizon, floor_start_y):
                    dist = (self.player.height_in_level * DIST) / (y - horizon + 1e-6)
                    world_x = pos_x + dist * cos_a; world_y = pos_y + dist * sin_a
                    map_x, map_y = int(world_x // TILE), int(world_y // TILE)
                    texture_to_use = default_floor_texture
                    custom_surface_data = self.custom_surfaces.get(self.player.floor, {}).get((map_x, map_y))
                    if custom_surface_data and custom_surface_data.get('floor'):
                        custom_tex_name = custom_surface_data['floor'].get('texture')
                        custom_texture = self.HORIZONTAL_SURFACES.get(custom_tex_name)
                        if custom_texture: texture_to_use = custom_texture
                    if texture_to_use:
                        tex_w, tex_h = texture_to_use.get_size()
            ...
            if ceil_end_y < horizon:
                for y in range(ceil_end_y, horizon):
                    dist = ((TILE - self.player.height_in_level) * DIST) / (horizon - y + 1e-6)
                    world_x = pos_x + dist * cos_a; world_y = pos_y + dist * sin_a
                    map_x, map_y = int(world_x // TILE), int(world_y // TILE)
                    texture_to_use_ceil = default_ceil_texture
