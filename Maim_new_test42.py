...
class Renderer:
    ...
    def draw_floor_and_ceiling(self):
        """
        FINALNA, POPRAWIONA WERSJA: Łączy precycyjne obliczenia wektorowe z optymalizacją
        "pixel skipping", aby poprawnie i wydajnie rysować podłogi/sufity bez zniekształceń.
        POPRAWIONO: Skalowanie niestandardowych podłóg w sposób zachowujący ostrość.
        """
        ...
        # RYSOWANIE PODŁOGI
        for y in range(horizon, RENDER_HEIGHT, PIXEL_SKIP):
            ...
            for x in range(0, RENDER_WIDTH, PIXEL_SKIP):
                ...
                is_custom_wide_floor = False
                
                custom_data = self.custom_surfaces.get(self.player.floor, {}).get((map_mx, map_my))
                if custom_data and custom_data.get('floor'):
                    custom_floor_info = custom_data['floor']
                    tex_name = custom_floor_info.get('texture')
                    custom_tex = self.HORIZONTAL_SURFACES.get(tex_name)
                    if custom_tex:
                        texture_to_use = custom_tex
                    if custom_floor_info.get('id') in [50, 51]:
                        is_custom_wide_floor = True
                
                if texture_to_use:
                    tex_w, tex_h = texture_to_use.get_size()
                    
                    # --- NOWA, STABILNA LOGIKA SKALOWANIA ---
                    if is_custom_wide_floor:
                        scale = 1.15  # Jak bardzo powiększyć teksturę (1.15 = 115%)
                        
                        # Zmieniamy wirtualny rozmiar kafelka. Tekstura powtórzy się
                        # na mniejszej przestrzeni, co da efekt powiększenia.
                        scaled_tile_size = TILE / scale
                        
                        # Używamy prostszej, bardziej stabilnej matematyki
                        tex_x = int((world_x % scaled_tile_size) / scaled_tile_size * tex_w)
                        tex_y = int((world_y % scaled_tile_size) / scaled_tile_size * tex_h)
                    else:
                        # Oryginalna kalkulacja dla wszystkich innych podłóg
                        tex_x = int(world_x % TILE / TILE * tex_w)
                        tex_y = int(world_y % TILE / TILE * tex_h)
                    # --- KONIEC NOWEJ LOGIKI ---

                    # Zabezpieczenie na wypadek, gdyby obliczenia wyszły poza zakres
                    tex_x = max(0, min(tex_x, tex_w - 1))
                    tex_y = max(0, min(tex_y, tex_h - 1))

                    color = texture_to_use.get_at((tex_x, tex_y))
                    pygame.draw.rect(self.screen, color, (x, y, PIXEL_SKIP, PIXEL_SKIP))
    
        # RYSOWANIE SUFITU (pozostaje bez zmian)
        for y in range(0, horizon, PIXEL_SKIP):
            ...
            for x in range(0, RENDER_WIDTH, PIXEL_SKIP):
                ...
                if texture_to_use:
                    ...
                    pygame.draw.rect(self.screen, color_to_use, (x, y, PIXEL_SKIP, PIXEL_SKIP))
