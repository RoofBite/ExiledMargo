...
SPRITE_PROPERTIES = {
    ...
    103: {
        ...
        "z": 0,
        "type": "pickup_item",
        "item_data": {"name": "Rózga", "value": 1, "type": "weapon", "attack": 1},
    },
}
...
# ==================================================================
#    ZASTĄP CAŁĄ SWOJĄ FUNKCJĘ game_loop_step TĄ WERSJĄ:
# ==================================================================
def game_loop_step(
    player,
    game_state,
    ...
    # Krok 1: Sprawdź, czy użytkownik chce zamknąć grę
    for e in pygame.event.get():
        if e.type == pygame.QUIT:
            return False  # Zakończ pętlę główną

    # Krok 2: Wyczyść cały ekran na CIEMNONIEBIESKI kolor
    # Używamy niebieskiego, aby mieć pewność, że to nasz kolor, a nie domyślny czarny.
    screen.fill((0, 0, 100))

    # Krok 3: Narysuj prosty, żółty kwadrat
    pygame.draw.rect(screen, (255, 255, 0), (100, 100, 300, 300))

    # Krok 4: Pokaż wynik na ekranie
    pygame.display.flip()

    # Krok 5: Powiedz pętli głównej, żeby kontynuowała działanie
    return True
...
def game_loop_step2(
    ...
    for e in pygame.event.get():
        ...
        if game_state.current_state == "playing":
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_w:
                    player.grid_move(True, sprites, game_state)
                if e.key == pygame.K_s:
                    player.grid_move(False, sprites, game_state)
                if e.key == pygame.K_a:
                    player.turn(True, game_state)
                if e.key == pygame.K_d:
                    player.turn(False, game_state)
                if e.key == pygame.K_e:
                    player.interact(sprites, game_state)
                if e.key == pygame.K_i:
                ...
                if attack_rect.collidepoint(mx, my):
                    ...
                    if player.moc > 0:
                        process_player_power_attack(
                            player, game_state.active_monster, game_state
                        )
                        game_state.monster_attack_timer = pygame.time.get_ticks()
                    else:
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, rect in enumerate(item_rects):
                    if rect.collidepoint(mx, my) and i < len(player.inventory):
                        player.manage_item(player.inventory[i], game_state)
                        break
    ...
    # TO JEST POPRAWNA LOGIKA
    if game_state.screen_dirty:
        # Wyczyść ekran przed rysowaniem nowej klatki świata 3D
        screen.fill((0, 0, 0))
        ...
        # Zapamiętaj ostatni render, aby nie rysować UI na starych klatkach
        last_world_render.blit(screen, (0, 0))

        game_state.screen_dirty = False
    else:
        # Jeśli scena się nie zmieniła, po prostu przywróć ostatnią klatkę
...
async def main():
    ...
    for filename in horizontal_files:
        try:
            ...
            # Po prostu ładujemy obrazek i go przechowujemy. Bez konwersji do NumPy.
            HORIZONTAL_TEXTURES[filename] = pygame.image.load(path).convert()
    ...
    #         HORIZONTAL_TEXTURES[filename] = pygame.surfarray.array3d(tex_raw).transpose(
    #             1, 0, 2
    #         )
    #     except (pygame.error, FileNotFoundError):
    #         print(f"Nie można załadować tekstury poziomej: {filename}")
...
class Renderer:
    ...
    def draw_floor_and_ceiling(self):
        # Ta wersja na razie rysuje tylko jednolite kolory, aby potwierdzić, że działa.
        is_outdoor = LEVEL_TEXTURES.get(self.player.floor, {}).get("is_outdoor", False)

        if is_outdoor:
            # Używamy definicji z LEVEL_TEXTURES lub domyślnego błękitu
            ceiling_color = LEVEL_TEXTURES.get(self.player.floor, {}).get(
                "ceiling", (135, 206, 235)
            )
            floor_color = (34, 139, 34)  # Zielony kolor trawy
        else:
            # Kolory dla wnętrz
            ceiling_color = (50, 50, 50)
            floor_color = (70, 70, 70)

        # Oblicz horyzont z uwzględnieniem patrzenia góra/dół
        horizon = RENDER_HEIGHT // 2 + int(self.player.pitch * RENDER_SCALE)

        # Narysuj sufit i podłogę na małej powierzchni render_surface
        # (Pamiętaj, że w Rendererze nazywa się ona self.screen)
        self.screen.fill(ceiling_color, (0, 0, RENDER_WIDTH, horizon))
        self.screen.fill(floor_color, (0, horizon, RENDER_WIDTH, RENDER_HEIGHT))

    def draw_floor_and_ceiling_old(self):
        is_outdoor = LEVEL_TEXTURES.get(self.player.floor, {}).get("is_outdoor", False)
        if is_outdoor:
    ...
    def draw_horizontal_surface(self, y_start, y_end, is_floor, step=1):
        ...
        default_res = (
            self.HORIZONTAL_TEXTURES.get(default_res_name)
        ...
        if not is_floor:
            ...
            floor_below_res = (
                self.HORIZONTAL_TEXTURES.get(floor_below_res_name)
                if isinstance(floor_below_res_name, str)
                else floor_below_res_name
            )
        ...
        for y in range(y_start, y_end, step):
            ...
            if customs:
                ...
                unique_visible_tiles = np.unique(
                    np.stack((map_x_coords, map_y_coords), axis=-1), axis=0
                )
                for tile in unique_visible_tiles:
                    if (tile[0], tile[1]) in customs and customs[
                        (tile[0], tile[1])
                    ].get("floor" if is_floor else "ceiling"):
            ...
            if customs:
                for tile in unique_visible_tiles:
                    ...
                    if (map_x, map_y) in customs:
                        surface_data = customs[(map_x, map_y)].get(
                            "floor" if is_floor else "ceiling"
                        )
                        if surface_data:
        ...
        for ray in range(NUM_RAYS):
            ...
            for _ in range(MAX_DEPTH):
                ...
                for block in sorted(wall_blocks, key=lambda b: b.get("z", 0)):
                    ...
                    horizon = (RENDER_HEIGHT // 2) + int(
                        self.player.pitch * RENDER_SCALE
                    )
...
def draw_character_screen_ui(screen, player, font, ui_font):
    ...
    for i, item in enumerate(player.inventory):
        ...
        color = (
            "yellow"
            if item.get("type") in ["weapon", "armor", "helmet", "shield"]
            else "lightgreen" if item.get("type") == "consumable" else "white"
        )
        draw_text(
            screen,
            f"- {item['name']}",
