...
class Renderer:
    ...
    def draw_horizontal_surface(self, y_start, y_end, is_floor, step=1):
        ...
        default_res_name = level_defaults.get("floor" if is_floor else "ceiling")
        ...
        for y in range(y_start, y_end, step):
            ...
            row_wx = np.linspace(start_x, end_x, RENDER_WIDTH)
            row_wy = np.linspace(start_y, end_y, RENDER_WIDTH)

            # 1. Narysuj domyślną płaszczyznę
            if default_res is not None:
                if isinstance(default_res, np.ndarray):
                    ...
                    self.pixel_buffer[y, :] = default_res[tex_y, tex_x]
                else:
                    self.pixel_buffer[y, :] = default_res

            # --- POCZĄTEK FINALNEJ OPTYMALIZACJI ---
            ...
            # Tworzymy tablicę par współrzędnych [x, y] dla całego rzędu
            coords = np.stack((map_x_coords, map_y_coords), axis=-1)
            
            # Znajdujemy UNIKALNE pary koordynatów - czyli tylko te kafelki, które widać w tym rzędzie
            unique_visible_tiles = np.unique(coords, axis=0)
            
            # Pętla wykonuje się teraz tylko tyle razy, ile unikalnych kafelków widać (np. 2-3 razy),
            # a nie tyle, ile jest wszystkich customów na mapie.
            for tile in unique_visible_tiles:
                map_x, map_y = tile[0], tile[1]
                
                # Sprawdzamy, czy ten konkretny widoczny kafelek jest na liście niestandardowych
                if (map_x, map_y) in customs:
                    surface_data_dict = customs[(map_x, map_y)]
                    surface_data = surface_data_dict.get('floor' if is_floor else 'ceiling')
                    if not surface_data: continue

                    # Tworzymy maskę tylko dla tego jednego, widocznego kafelka
                    ...
                    if tex_array is not None:
                        ...
                        masked_wx = row_wx[mask]
                        masked_wy = row_wy[mask]
                        
                        tex_coord_x = (masked_wx * tex_w).astype(int) & (tex_w - 1)
                        tex_coord_y = (masked_wy * tex_h).astype(int) & (tex_h - 1)
                        
                        new_colors = tex_array[tex_coord_y, tex_coord_x]

                        self.pixel_buffer[y, mask] = new_colors
            # --- KONIEC FINALNEJ OPTYMALIZACJI ---
    ...
    def draw_horizontal_surface2(self, y_start, y_end, is_floor, step=1):
        ...
        for y in range(y_start, y_end, step):
            ...
            # 1. Narysuj domyślną płaszczyznę (szybka operacja)
            if default_res is not None:
                if isinstance(default_res, np.ndarray):
                    tex_h, tex_w = default_res.shape[:2]
                    tex_x = (row_wx * tex_w).astype(int) & (tex_w - 1)
            ...
            # 2. Na wierzchu narysuj niestandardowe płaszczyzny w sposób zoptymalizowany
            customs = self.custom_surfaces.get(self.player.floor, {})
            if not customs: continue

            # Obliczamy koordynaty mapy dla CAŁEGO rzędu za jednym zamachem
            map_x_coords = row_wx.astype(int)
            map_y_coords = row_wy.astype(int)

            # Zamiast pętli po pikselach, robimy pętlę po niestandardowych kafelkach (których jest mało)
            for (map_x, map_y), surface_data_dict in customs.items():
                surface_data = surface_data_dict.get('floor' if is_floor else 'ceiling')
                if not surface_data: continue

                # Tworzymy "maskę" - tablicę True/False, która mówi nam, które piksele leżą na naszym kafelku
                mask = (map_x_coords == map_x) & (map_y_coords == map_y)

                # Jeśli jakikolwiek piksel w rzędzie pasuje do maski...
                if np.any(mask):
                    tex_name = surface_data.get('texture')
                    tex_array = self.HORIZONTAL_TEXTURES.get(tex_name)
                    if tex_array is not None:
                        ...
                        # Wykonujemy obliczenia tylko dla pikseli, które pasują do maski
                        masked_wx = row_wx[mask]
                        masked_wy = row_wy[mask]
                        ...
                        # Używamy tej samej maski, aby podmienić piksele w buforze na nowe kolory
                        self.pixel_buffer[y, mask] = new_colors
