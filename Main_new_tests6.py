...
# NOWA STRUKTURA: Przechowuje tekstury podłogi i sufitu dla każdego piętra
LEVEL_TEXTURES = {
    # format --> numer_piętra: {"ceiling": "sufit.png", "floor": "podloga.png"}
    0: {"ceiling": (50,50,150), "floor": "wall17.png"},      # Poziom startowy
    1: {"ceiling": "", "floor": "wall17.png"},       # Korytarze z drewnianą podłogą
   -1: {"ceiling": "wall17.png", "floor": "wall17.png"},     # Lochy z kamienną podłogą
    2: {"ceiling": "", "floor": ""}        # Las z ziemistą podłogą
}
...
# Zakładając, że stałe (RENDER_WIDTH, RENDER_HEIGHT, etc.) są zdefiniowane globalnie
...
class Renderer:
    def __init__(self, screen, player, maps, walls, sprites):
        ...
        # ZMIANA: Zamiast *_NP przechowujemy ogólne zasoby (tekstury lub kolory)
        self.FLOOR_RESOURCES = {}
        self.CEILING_RESOURCES = {}

        for floor_index, resources in LEVEL_TEXTURES.items():
            # Przetwarzanie zasobu dla podłogi
            floor_res = resources.get("floor")
            if isinstance(floor_res, str): # Jeśli to string, ładujemy teksturę
                try:
                    path = os.path.join(TEXTURE_PATH, floor_res)
                    tex_raw = pygame.image.load(path).convert()
                    self.FLOOR_RESOURCES[floor_index] = pygame.surfarray.array3d(tex_raw)
                except (pygame.error, FileNotFoundError):
                    self.FLOOR_RESOURCES[floor_index] = (100, 80, 50) # Kolor awaryjny
            elif isinstance(floor_res, (tuple, list)): # Jeśli to tuple/lista, zapisujemy jako kolor
                self.FLOOR_RESOURCES[floor_index] = floor_res

            # Przetwarzanie zasobu dla sufitu
            ceil_res = resources.get("ceiling")
            if isinstance(ceil_res, str): # Jeśli to string, ładujemy teksturę
                try:
                    path = os.path.join(TEXTURE_PATH, ceil_res)
                    tex_raw = pygame.image.load(path).convert()
                    self.CEILING_RESOURCES[floor_index] = pygame.surfarray.array3d(tex_raw)
                except (pygame.error, FileNotFoundError):
                    self.CEILING_RESOURCES[floor_index] = (50, 50, 50) # Kolor awaryjny
            elif isinstance(ceil_res, (tuple, list)): # Jeśli to tuple/lista, zapisujemy jako kolor
                self.CEILING_RESOURCES[floor_index] = ceil_res

        self.pixel_buffer = np.zeros((RENDER_WIDTH, RENDER_HEIGHT, 3), dtype=np.uint8)
    ...
    def draw_floor_and_ceiling(self):
        ...
        horizon_y_abs = render_half_height + int(self.player.pitch * RENDER_SCALE)

        # Dynamiczne wybieranie zasobów
        ...
        for y in range(RENDER_HEIGHT):
            # Wybór odpowiedniego zasobu i logiki
            res, is_floor = (floor_res, True) if y > horizon_y_abs else (ceil_res, False)

            # Jeśli zasób nie istnieje, pomiń
            if res is None: continue

            # SPRAWDZANIE TYPU: Czy rysujemy teksturę czy jednolity kolor?
            if isinstance(res, np.ndarray): # To jest tekstura (tablica numpy)
                ...
                ray_dir_x0 = dir_x - plane_x; ray_dir_y0 = dir_y - plane_y
                ray_dir_x1 = dir_x + plane_x; ray_dir_y1 = dir_y + plane_y
                
                start_x = pos_x + row_distance * ray_dir_x0
                start_y = pos_y + row_distance * ray_dir_y0
                end_x = pos_x + row_distance * ray_dir_x1
                end_y = pos_y + row_distance * ray_dir_y1
                ...
                colors = res[tex_coords_y, tex_coords_x] # Poprawiona kolejność indeksowania
                self.pixel_buffer[:, y] = colors

            elif isinstance(res, (tuple, list)): # To jest kolor
                self.pixel_buffer[:, y] = res

        pygame.surfarray.blit_array(self.screen, self.pixel_buffer)
    ...
    def draw_walls(self):
        ...
        for ray in range(NUM_RAYS):
            ...
            for _ in range(MAX_DEPTH):
                ...
                if 0 <= ray < NUM_RAYS: self.z_buffer[ray] = depth
    ...
    def draw_sprites(self):
        ...
        for spr in self.sprites:
            ...
            sprites_with_depth.append((spr, dist_corr, gamma))
