...
class WeatherManager:
    def __init__(self, rain_manager):
        self.rain_manager = rain_manager
        self.is_raining = False
        
        # Definicja stanów i przejść (Łańcuch Markowa)
        self.states = ["Słonecznie", "Pochmurno", "Deszczowo"]
        self.transitions = {
            "Słonecznie": {"Słonecznie": 0.70, "Pochmurno": 0.25, "Deszczowo": 0.05},
            "Pochmurno":  {"Słonecznie": 0.40, "Pochmurno": 0.40, "Deszczowo": 0.20},
            "Deszczowo":  {"Słonecznie": 0.10, "Pochmurno": 0.60, "Deszczowo": 0.30}
        }
        
        # Stan początkowy
        self.current_state = "Słonecznie"
        print(f"INFO: Pogoda startuje jako: {self.current_state}")

    def trigger_new_weather_period(self):
        """
        Główna funkcja logiki. Oblicza następny stan pogody na podstawie obecnego.
        """
        # Pobierz możliwe przejścia dla aktualnego stanu
        possible_next_states = self.transitions[self.current_state]
        
        # Wylosuj następny stan na podstawie wag (prawdopodobieństw)
        ...
        print(f"INFO: Pogoda zmieniła się na: {self.current_state}")
        
        # Zaktualizuj flagę deszczu
        self.is_raining = (self.current_state == "Deszczowo")

    def update(self):
        """Aktualizowana w każdej klatce gry."""
        if self.is_raining:
            self.rain_manager.update()

    def draw(self, surface):
        """Rysuje deszcz, jeśli pada."""
        if self.is_raining:
...
class DayNightManager:
    ...
    def update(self, game_state): # <-- WAŻNA ZMIANA: Dodajemy game_state
        ...
        time_fraction = (pygame.time.get_ticks() % self.cycle_duration) / self.cycle_duration
        ...
        if darkness_level < self.previous_darkness_level and not self.respawn_triggered_this_cycle and darkness_level > 0.95:
...
def game_loop_step(player, game_state, renderer, sprites, screen, render_surface, clock, font, ui_font, info_font, sprite_properties, sprite_textures, last_world_render, logger, rain_manager, day_night_manager,weather_manager):
    dt = clock.tick(50)
        ...
        for floor_idx, monster_list in eligible_by_floor.items():
            ...
            if are_respawns_allowed_on_floor(floor_idx, player, game_state, sprites):
                ...
                if num_to_respawn > 0:
                    ...
                    print(f"INFO: Północ. Na piętrze {floor_idx} wybrano {len(monsters_to_respawn)} potworów do odrodzenia.")
                ...
                print(f"INFO: Północ. Odradzanie na piętrze {floor_idx} zostało zablokowane przez warunek specjalny.")
...
async def main():
    ...
    weather_manager = WeatherManager(rain_manager)
    ...
    while running:
        ...
        running = game_loop_step(player, game_state, renderer, sprites, screen, render_surface, clock, font, ui_font, info_font, SPRITE_PROPERTIES, SPRITE_TX, last_world_render, logger, rain_manager, day_night_manager, weather_manager)
...
class Renderer:
    def __init__(self, screen, player, maps, walls, sprites, HORIZONTAL_TEXTURES, PLANE_PROPERTIES, SPRITE_PROPERTIES):
        ...
        print("INFO: Przetwarzanie wstępne niestandardowych płaszczyzn w siatkę przestrzenną...")
        self.custom_surfaces_grid = {} 
        
        for floor_idx, world_map in maps.items():
            map_height = len(world_map)
            if map_height == 0: continue
    ...
    def draw_floor_and_ceiling_pygame(self):
        ...
        for y in range(horizon, RENDER_HEIGHT):
            ...
            start_wx = pos_x + dist * left_vec_x; start_wy = pos_y + dist * left_vec_y
            end_wx = pos_x + dist * right_vec_x; end_wy = pos_y + dist * right_vec_y
            step_x = (end_wx - start_wx) / RENDER_WIDTH; step_y = (end_wy - start_wy) / RENDER_WIDTH
            for x in range(0, RENDER_WIDTH, PIXEL_SKIP):
                world_x = start_wx + x * step_x; world_y = start_wy + x * step_y
                        ...
                        if 0 <= ny < grid_h and 0 <= nx < grid_w:
                            ...
                            if cell_data and cell_data.get('floor'):
                                ...
                                if (nx * TILE - pad * TILE <= world_x <= (nx + 1) * TILE + pad * TILE and
                                    ...
                                    center_x = nx * TILE + TILE / 2; center_y = ny * TILE + TILE / 2
        ...
        for y in range(horizon):
            ...
            for x in range(0, RENDER_WIDTH, PIXEL_SKIP):
                world_x = start_wx + x * step_x; world_y = start_wy + x * step_y
                map_mx, map_my = int(world_x // TILE), int(world_y // TILE)
                texture_to_use = default_ceil_texture; color_to_use = default_ceil_color
                if custom_grid and 0 <= map_my < grid_h and 0 <= map_mx < grid_w:
                    ...
                    if cell_data and cell_data.get('ceiling'):
                        ...
                        custom_tex = self.HORIZONTAL_SURFACES.get(tex_name)
                        if custom_tex: 
                            texture_to_use = custom_tex; color_to_use = None
                if texture_to_use:
                    tex_w, tex_h = texture_to_use.get_size()
                    tex_x = int(world_x / TILE * tex_w) % tex_w; tex_y = int(world_y / TILE * tex_h) % tex_h
                    color = texture_to_use.get_at((tex_x, tex_y))
                    pygame.draw.rect(self.screen, color, (x, y, PIXEL_SKIP, 1))
                elif color_to_use:
    ...
    def draw_horizontal_surface_numpy(self, y_start, y_end, is_floor, step=1):
        ...
        for y in range(y_start, y_end, step):
            ...
            if custom_grid:
                ...
                for map_x, map_y in np.unique(np.stack((map_x_coords, map_y_coords), axis=-1), axis=0):
                    if 0 <= map_y < grid_h and 0 <= map_x < grid_w:
                        cell_data = custom_grid[map_y][map_x]
                        if cell_data:
                            surface_data = cell_data.get('floor' if is_floor else 'ceiling')
                            if surface_data:
...
def are_respawns_allowed_on_floor(floor_index, player, game_state, all_sprites):
    ...
    for spr in all_sprites:
        if spr.floor == floor_index and not spr.is_dead and spr.properties.get("no_respawn"):
...
def process_player_power_attack(player, monster, game_state,logger):
    ...
    monster.hp -= dmg
    logger.info(f"PLAYER_ATTACK (MOC); Target: {monster.name}; Damage: {dmg}; MonsterHP: {monster.hp}/{monster.max_hp}")
