...
def game_loop_step(player, game_state, renderer, sprites, screen, render_surface, clock, font, ui_font, info_font, sprite_properties, sprite_textures):
    dt = clock.tick(30)
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                ...
                if leave_rect.collidepoint(mx, my): game_state.end_dialogue()
                    ...
                    if 'accept_quest' in action_rects and action_rects['accept_quest'].collidepoint(mx, my):
                        ...
                        # Wywołaj funkcję zamieniającą szczury na beczki
                        replace_quest_rats_with_barrels(sprites, sprite_properties, sprite_textures)
                        # <<< ------------------------------------ >>>
...
async def main():
    ...
    WALLS = {k: pygame.image.load(os.path.join(TEXTURE_PATH, f)).convert() for k, f in {1:'wall5.png', 2:'wall2.png', 97:'wall17.png', 95:'wall12.png' ,96: 'door_closed_wall.png',10:'stairs_up.png', 11:'stairs_down.png'}.items()}
            ...
            for rx, vals in enumerate(row):
                ...
                for v in vals:
                    ...
                    if sprite_id in SPRITE_PROPERTIES:
                        ...
                        # 1. Stwórz KOPIĘ domyślnych właściwości, aby ich nie nadpisać globalnie
                        final_props = SPRITE_PROPERTIES[sprite_id].copy()
                        
                        # 2. Zaktualizuj kopię właściwościami z mapy (np. z: 1 nadpisze domyślne z: 0)
                        final_props.update(v)

                        tex = SPRITE_TX[sprite_id]
                        
                        # 3. Przekaż do sprajta finalny, połączony zestaw właściwości
    ...
    while running:
        running = game_loop_step(player, game_state, renderer, sprites, screen, render_surface, clock, font, ui_font, info_font, SPRITE_PROPERTIES, SPRITE_TX)
...
class Renderer:
    ...
    def draw_horizontal_surface2(self, y_start, y_end, is_floor, step=1):
        ...
        # --- POCZĄTEK ZMIAN: Logika dla rysowania "przez dziury" ---
        
        # Pobieramy zasoby dla aktualnego piętra
        ...
        # Jeśli rysujemy sufit, pobieramy też teksturę PODŁOGI z piętra NIŻEJ
        if not is_floor:
        ...
        for y in range(y_start, y_end, step):
            ...
            # Ta matematyka pozostaje bez zmian
            if is_floor:
            ...
            row_wx = np.linspace(pos_x + row_distance * (dir_x - plane_x), pos_x + row_distance * (dir_x + plane_x), RENDER_WIDTH)
            row_wy = np.linspace(pos_y + row_distance * (dir_y - plane_y), pos_y + row_distance * (dir_y + plane_y), RENDER_WIDTH)

            # --- POCZĄTEK ZMIAN: Rysowanie warstwowe ---

            # 1. Rysowanie tła
            if not is_floor and floor_below_res is not None:
                # Jeśli rysujemy sufit, naszym tłem jest podłoga z piętra niżej
                if isinstance(floor_below_res, np.ndarray):
                    ...
                    self.pixel_buffer[y, :] = floor_below_res[tex_y, tex_x]
                else:
                    self.pixel_buffer[y, :] = floor_below_res
            
            # 2. Rysowanie domyślnej płaszczyzny (z "dziurami" na customowe)
            customs = self.custom_surfaces.get(self.player.floor, {})
            # Tworzymy maskę, która mówi, gdzie są niestandardowe kafelki - tam nie będziemy rysować domyślnego sufitu/podłogi
            ...
            # Rysujemy domyślną płaszczyznę tylko tam, gdzie nie ma customowej (używając odwróconej maski ~)
            if default_res is not None:
                if isinstance(default_res, np.ndarray):
                    ...
                    self.pixel_buffer[y, ~master_mask] = default_res[tex_y, tex_x]
                else:
                    self.pixel_buffer[y, ~master_mask] = default_res

            # 3. Na wierzchu rysujemy niestandardowe płaszczyzny
            if customs:
                # Ponownie używamy pętli po widocznych kafelkach, ale tym razem do rysowania
                for tile in unique_visible_tiles:
                    map_x, map_y = tile[0], tile[1]
                    if (map_x, map_y) in customs:
                        surface_data = customs[(map_x, map_y)].get('floor' if is_floor else 'ceiling')
                        if not surface_data: continue
                        ...
                        if tex_array is not None:
                            texture_scale = surface_data.get('texture_scale', 1.0)
                            tex_h, tex_w = tex_array.shape[:2]
                            tex_coord_x = (row_wx[mask] * tex_w * texture_scale).astype(int) & (tex_w - 1)
                            tex_coord_y = (row_wy[mask] * tex_h * texture_scale).astype(int) & (tex_h - 1)
                            self.pixel_buffer[y, mask] = tex_array[tex_coord_y, tex_coord_x]
            # --- KONIEC ZMIAN ---
    ...
    def draw_horizontal_surface(self, y_start, y_end, is_floor, step=1):
        ...
        for y in range(y_start, y_end, step):
            ...
            # Domyślne obliczenia dla standardowej podłogi/sufitu
            if is_floor:
                 cam_z = 0.5 * self.player.height_in_level / TILE
            else:
                 cam_z = 0.5 * (TILE - self.player.height_in_level) / TILE
            ...
            # 1. Rysowanie tła (ważne dla "dziur w podłodze")
            if not is_floor and floor_below_res is not None:
                if isinstance(floor_below_res, np.ndarray):
                    tex_h, tex_w = floor_below_res.shape[:2]
                    tex_x = (row_wx * tex_w).astype(int) & (tex_w - 1)
            ...
            # 2. Rysowanie domyślnej płaszczyzny
            customs = self.custom_surfaces.get(self.player.floor, {})
            master_mask = np.zeros(RENDER_WIDTH, dtype=bool)
            if customs:
                map_x_coords_master = row_wx.astype(int)
                map_y_coords_master = row_wy.astype(int)
                unique_visible_tiles_master = np.unique(np.stack((map_x_coords_master, map_y_coords_master), axis=-1), axis=0)
                for tile in unique_visible_tiles_master:
                    if (tile[0], tile[1]) in customs and customs[(tile[0], tile[1])].get('floor' if is_floor else 'ceiling'):
                         mask = (map_x_coords_master == tile[0]) & (map_y_coords_master == tile[1])
                         master_mask |= mask
            ...
            # 3. Rysowanie niestandardowych płaszczyzn z poprawną perspektywą
            if customs:
                for tile in unique_visible_tiles_master:
                    map_x, map_y = tile[0], tile[1]
                    if (map_x, map_y) in customs:
                        ...
                        # --- POCZĄTEK NOWEJ LOGIKI KOREKCJI PERSPEKTYWY ---
                        
                        # Pobieramy wysokość 'z' tego konkretnego kafelka
                        z_offset = surface_data.get('z', 1 if not is_floor else 0)

                        # Obliczamy prawdziwą pionową odległość kamery od tej płaszczyzny
                        player_height_relative = self.player.height_in_level
                        plane_height_relative = z_offset * TILE
                        
                        cam_z_correct = abs(player_height_relative - plane_height_relative)
                        if cam_z_correct == 0: continue

                        # Używamy tego samego skalowania co w domyślnych obliczeniach dla spójności
                        final_cam_z = 0.5 * cam_z_correct / TILE
                        row_distance_correct = final_cam_z * (PROJ_COEFF / TILE) / p

                        # Obliczamy na nowo współrzędne w świecie dla tej JEDNEJ płaszczyzny
                        row_wx_correct = np.linspace(pos_x + row_distance_correct * (dir_x - plane_x), pos_x + row_distance_correct * (dir_x + plane_x), RENDER_WIDTH)
                        row_wy_correct = np.linspace(pos_y + row_distance_correct * (dir_y - plane_y), pos_y + row_distance_correct * (dir_y + plane_y), RENDER_WIDTH)
                        
                        # --- KONIEC NOWEJ LOGIKI ---
                        
                        mask = (map_x_coords_master == map_x) & (map_y_coords_master == map_y)
                        tex_name = surface_data.get('texture')
                        tex_array = self.HORIZONTAL_TEXTURES.get(tex_name)
                        ...
                        if tex_array is not None:
                            ...
                            # Używamy nowo obliczonych, poprawnych współrzędnych
                            tex_coord_x = (row_wx_correct[mask] * tex_w * texture_scale).astype(int) & (tex_w - 1)
                            tex_coord_y = (row_wy_correct[mask] * tex_h * texture_scale).astype(int) & (tex_h - 1)
                            
                            self.pixel_buffer[y, mask] = tex_array[tex_coord_y, tex_coord_x]
    

    # Wklej tę wersję do swojej aktualnej klasy Renderer
    def draw_sprites(self):
        ...
        for spr, dist_corr, gamma, theta in sprites_with_depth:
            ...
            # Ta część kodu (związana z billboardem i rysowaniem) pozostaje bez zmian
        ...
        for ray in range(NUM_RAYS):
            ...
            for _ in range(MAX_DEPTH):
                ...
                for block in sorted(wall_blocks, key=lambda b: b.get('z', 0)):
                    ...
                    # --- POPRAWIONA I BARDZIEJ CZYTELNA LOGIKA WYSOKOŚCI ---
                    z_offset = block.get('z', 0)
                    wall_height = block.get("h", 1.0) # Domyślnie pełna wysokość
                    
                    h_wall_bottom_rel = z_offset * TILE
                    # Kluczowa poprawka: wysokość ściany jest dodawana do jej dolnej pozycji
                ...
                # Jeśli trafiliśmy na ścianę z listy SOLID_WALLS, która jest na poziomie gruntu (z=0)
                # i ma standardową wysokość (h=1.0), to nic za nią nie będzie widać.
                # Możemy bezpiecznie przerwać dalsze poszukiwania dla tego promienia.
                if any(b.get('id') in self.SOLID_WALLS and b.get('z', 0) == 0 and b.get('h', 1.0) >= 1.0 for b in wall_blocks):
                    break
            
            # Sortowanie i rysowanie pozostaje bez zmian, ale teraz operuje na znacznie mniejszej liczbie segmentów
            ...
            for _, y_top, seg_h, raw_col, x1, w in column_segments:
                if seg_h > 0 and w > 0:
                    ...
                    except ValueError: # Ignoruj błędy skalowania do zerowych wymiarów
                        pass
    
            cur_angle += DELTA_ANGLE

    def draw_walls2(self):
        self.z_buffer = [float('inf')] * NUM_RAYS# RENDER_WIDTH
        current_map = self.maps[self.player.floor]
        cur_angle = self.player.angle - HALF_FOV
        # --- DODANA ZMIENNA ---
        # Obliczamy, jak szeroka powinna być każda kolumna na ekranie
        column_width = RENDER_WIDTH / NUM_RAYS
        ...
        for ray in range(NUM_RAYS):
            ...
            for _ in range(MAX_DEPTH):
                ...
                # To jest kluczowa linia do korekcji "rybiego oka"
                depth_corr = depth * math.cos(self.player.angle - cur_angle)
                ...
                for block in sorted(wall_blocks, key=lambda b: b.get('z', 0)):
                    ...
                    # --- NOWA LOGIKA POZYCJI I SZEROKOŚCI ---
                    # Obliczamy pozycję X na ekranie i szerokość tej kolumny
                    x1 = int(ray * column_width)
                    w = max(1, int((ray + 1) * column_width) - x1)

                    tex_id, z_off = block['id'], block.get('z', 0)
                    tex = self.walls[tex_id]
                    proj_h = PROJ_COEFF / (depth_corr + 1e-6)
...
# --- K
                    ...
                    if seg_h > 0:
                        off = int((wx % TILE) / TILE * tex.get_width())
                        raw_col = tex.subsurface(off, 0, 1, tex.get_height())
                        # Przekazujemy obliczone x1 i w do danych segmentu
                        segment_data = (depth_corr, y_top, seg_h, raw_col, x1, w)
                        column_segments.append(segment_data)
    
                if any(b.get('id') in self.SOLID_WALLS for b in wall_blocks):
                    break
            
            column_segments.sort(key=lambda x: x[0], reverse=True)
            
            # --- POPRAWIONE RYSOWANIE ---
            # Używamy x1 i w do skalowania i rysowania kolumny w odpowiednim miejscu
            for depth, y_top, seg_h, raw_col, x1, w in column_segments:
                col = pygame.transform.scale(raw_col, (w, int(seg_h)))
                self.screen.blit(col, (x1, y_top))
...
def replace_quest_rats_with_barrels(sprites, sprite_properties, sprite_textures):
    """
    ...
    # Pobierz wzorcowe właściwości i teksturę dla beczki
    barrel_id = 24
    try:
...
def draw_combat_ui(screen, player, monster, game_state, font, info_font, ui_font):
    ...
    panic_flee_rect = pygame.Rect(SCREEN_WIDTH - 400, SCREEN_HEIGHT - 170, 300, 80) # Nowy przycisk
    ...
    pygame.draw.rect(screen, pygame.Color('purple4'), panic_flee_rect, border_radius=12) # Rysowanie nowego przycisku

    draw_text(screen, "Atakuj", attack_rect.center, font, center=True)
    draw_text(screen, "Wycofaj się", flee_rect.center, font, center=True)
    # Użycie mniejszej czcionki dla dłuższego tekstu
    draw_text(screen, "Paniczna Ucieczka", panic_flee_rect.center, ui_font, center=True) 
    
    # Zwróć prostokąt nowego przycisku
