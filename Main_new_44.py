...
class Renderer:
    ...
    def draw_floor_and_ceiling(self):
        """
        FINALNA, POPRAWIONA WERSJA: Łączy precyzyjne obliczenia wektorowe z optymalizacją
        ...
        for y in range(horizon, RENDER_HEIGHT):
            ...
            # Oblicz współrzędne świata dla skrajnie lewego i prawego piksela w rzędzie
            start_wx = pos_x + dist * left_vec_x
            start_wy = pos_y + dist * left_vec_y
            end_wx = pos_x + dist * right_vec_x
            end_wy = pos_y + dist * right_vec_y
            
            # Oblicz krok interpolacji dla każdego piksela w rzędzie
            ...
            for x in range(0, RENDER_WIDTH, PIXEL_SKIP):
                ...
                # domyślna tekstura pozioma
                texture_to_use = default_floor_texture

                # --- nowa logika: sprawdzamy kafelki sąsiednie i rozszerzenie (padding) ---
                candidates = []
                custom_map = self.custom_surfaces.get(self.player.floor, {})

                for ox in (-1, 0, 1):
                    for oy in (-1, 0, 1):
                        nx, ny = map_mx + ox, map_my + oy
                        cell = custom_map.get((nx, ny))
                        if not cell:
                            continue
                        floor_props = cell.get('floor')
                        if not floor_props:
                            continue
                        # padding w ułamku (np. 0.2 = 20%)
                        pad = float(floor_props.get('padding', 0.0))

                        # współrzędne lokalne względem początku kafelka (nx, ny)
                        local_x = world_x - nx * TILE
                        local_y = world_y - ny * TILE

                        # rozszerzony zakres (z każdej strony o pad * TILE)
                        if -pad * TILE <= local_x <= TILE + pad * TILE and -pad * TILE <= local_y <= TILE + pad * TILE:
                            # obliczamy dystans do środka kafelka żeby rozstrzygnąć konflikty
                ...
                # jeżeli znaleźliśmy kandydatów, wybieramy tego najbliższego środkowi kafelka
                if candidates:
                    candidates.sort(key=lambda t: t[0])
                    chosen_floor = candidates[0][1]
                    tex_name = chosen_floor.get('texture')
