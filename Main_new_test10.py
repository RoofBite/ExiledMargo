...
async def main():
    ...
    # Wewnątrz funkcji main(), znajdź ten fragment i go podmień:
...
class Renderer:
    ...
    def draw_sprites(self):
        ...
        for spr, dist_corr, gamma, theta in sprites_with_depth:
            ...
            if not is_billboard:
                ...
                # 1. Poprawne zdefiniowanie kąta normalnej ściany
                sprite_plane_angle = math.pi / 2 if orientation == 'x' else 0
                
                # 2. Obliczenie kąta między WZROKIEM gracza a płaszczyzną ściany
                angle_between_view_and_normal = self.player.angle - sprite_plane_angle
                
                # 3. Poprawne obliczenie współczynnika deformacji (spłaszczenia)
    ...
    def draw_sprites2(self):
        ...
        for spr, dist_corr, gamma, theta in sprites_with_depth:
            ...
            # Przywrócona, poprawna logika do obsługi perspektywy
            if not is_billboard:
                orientation = props.get("orientation", "x")
    ...
    def draw_walls(self):
        ...
        for ray in range(NUM_RAYS):
            ...
            # <<< ZMIANA 1: Bardziej stabilne obliczanie odległości delta >>>
            delta_x = float('inf') if cos_a == 0 else abs(TILE / cos_a)
            delta_y = float('inf') if sin_a == 0 else abs(TILE / sin_a)

            step_x, step_y = (1 if cos_a > 0 else -1), (1 if sin_a > 0 else -1)
            
            # <<< ZMIANA 2: Bezpieczne obliczanie odległości do krawędzi kafelka >>>
            if cos_a == 0:
                side_dx = float('inf')
            else:
                side_dx = ((x_map + 1) * TILE - ox) / cos_a if cos_a > 0 else (x_map * TILE - ox) / cos_a
            
            if sin_a == 0:
                side_dy = float('inf')
            else:
                side_dy = ((y_map + 1) * TILE - oy) / sin_a if sin_a > 0 else (y_map * TILE - oy) / sin_a
