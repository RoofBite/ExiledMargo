...
import sys
import math
import random # Potrzebne dla przyszłych rozszerzeń
...
# *** NOWY, ZAKTUALIZOWANY SŁOWNIK WŁAŚCIWOŚCI SPRITE'ÓW ***
# Łączy informacje z edytora i gry, dając pierwszeństwo danym z gry (z kluczem "texture")
SPRITE_PROPERTIES = {
    # Podstawowe elementy z edytora (bez tekstur w grze)
    ...
    # Dane z gry (zastępują lub uzupełniają te z edytora)
    ...
    46: {"texture": "stairs_up.png", "type": "portal", "name": "Schody góra (generyczne)"},
    47: {"texture": "stairs_down.png", "type": "portal", "name": "Schody dół (generyczne)"},
...
PALETTE_ITEMS = {
    "Gumka": "ERASER", "Ściana (w)": 'w', "Ściana Piwnicy (w2)": 'w2', "Szczur (rat)": 'rat',"Silny szczur (rat2)": 'rat2', "Jeleń (deer)": 'deer',
    ...
    "Niest. Sufit (60)": {"id": 60, "z": 0}, "Ściana Pozioma": {"id": 98, "z": 0}, "Ściana Pionowa": {"id": 99, "z": 0}, "Drzwi": {"id": 96, "z": 0, "target": 0}
}
# === FUNKCJE POMOCNICZE ===
...
class MapEditorTouch:
    def __init__(self):
        ...
        pygame.display.set_caption("Edytor Map (Landscape, v5) - Z Grafikami z Gry")
    ...
    # *** ZAKTUALIZOWANA FUNKCJA ŁADOWANIA GRAFIK ***
    def load_sprites(self):
        """Ładuje obrazki z folderu 'sprites' na podstawie klucza 'texture' w SPRITE_PROPERTIES."""
        sprite_folder = "assets/textures/"
        if not os.path.isdir(sprite_folder):
            print(f"Ostrzeżenie: Nie znaleziono folderu '{sprite_folder}'. Grafiki nie zostaną załadowane.")
            return
            ...
            if "texture" in properties:
                ...
                try:
                    if os.path.exists(filepath):
                        ...
                        # print(f"Załadowano grafikę dla ID {obj_id}: {filepath}") # Odkomentuj do debugowania
                    else:
                        # print(f"Ostrzeżenie: Brak pliku grafiki dla ID {obj_id}: {filepath}") # Odkomentuj do debugowania
                        pass
    ...
    def get_portal_target_from_user(self):
        input_box1 = InputBox(400, 200, 140, 32, self.font_medium, "")
        input_box2 = InputBox(400, 250, 140, 32, self.font_medium, "")
        input_box3 = InputBox(400, 300, 140, 32, self.font_medium, "")
        ...
        while not done:
            ...
            for i, box in enumerate(input_boxes):
                label_surface = self.font_medium.render(labels[i], True, (255, 255, 255))
                self.screen.blit(label_surface, (box.rect.x - 100, box.rect.y + 5))
                box.draw(self.screen)
            info_text = self.font_medium.render("Wpisz wartości i naciśnij ENTER w ostatnim polu", True, (200, 200, 200))
            self.screen.blit(info_text, (200, 400))
            pygame.display.flip()
            self.clock.tick(30)
        try:
            return int(input_boxes[0].text), int(input_boxes[1].text), int(input_boxes[2].text)
        except ValueError:
            return None
            ...
            for x in range(start_x, end_x):
                ...
                for obj_to_draw in visible_objects:
                    ...
                    if isinstance(obj_to_draw, str) and obj_to_draw in globals(): 
                        obj_id = globals()[obj_to_draw].get('id')

                    if obj_id in self.sprite_images:
                        sprite = self.sprite_images[obj_id]
                ...
                if visible_objects:
                    ...
                    top_obj_id = top_object.get('id') if isinstance(top_object, dict) else None
                    top_obj_z = top_object.get('z', 0) if isinstance(top_object, dict) else 0

                    if effective_tile_size > 40:
                        id_text = f"ID:{top_obj_id}" if top_obj_id is not None else '?'
                        self.screen.blit(self.font_small.render(id_text, True, COLOR_TEXT), (rect.x + 5, rect.y + 5))
                        z_text = f"Z:{top_obj_z}"
    ...
    def draw_ui(self):
        ...
        current_mode_text = mode_texts[self.get_edit_mode()]
    ...
    def draw_dialog(self):
        ...
        if self.dialog_mode in ['new_map', 'resize_map']:
            for i, (label, val) in enumerate([("Szerokość", self.dialog_map_width), ("Wysokość", self.dialog_map_height)]):
                y_pos = box_rect.y + 150 + i * 100
                prefix = 'w' if i == 0 else 'h'
    ...
    def load_map_from_csv(self, filename):
        if not (filename.endswith(".csv") and os.path.exists(filename)):
