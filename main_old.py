...
SPRITE_PROPERTIES = {
    ...
    40: {"texture": "stairs_up.png", "scale_x": 1.0, "scale_y": 1.0, "blocking": True, "z": 0, "type": "portal"},
    41: {"texture": "stairs_down.png", "scale_x": 1.0, "scale_y": 1.0, "blocking": True, "z": 0, "type": "portal"},
    ...
    22: {"texture": "rat_king.png", "scale_x": 0.5, "scale_y": 0.5, "blocking": True, "z": -15, # Boss
...
### POPRAWKA: Nowe, hybrydowe szablony dla portali ###
#w = {"id": 1, "z": 0}          # Zwykła ściana
#s1 = {"id": 5, "z": 0}         # Drzewo
#rat = {"id": 3, "z": 0}        # Szczur
#merchant = {"id": 20, "z": 0}  # Handlarz
#healer = {"id": 21, "z": 0}    # Uzdrowicielka
#deer = {"id": 14, "z": 0}      # Jeleń
#rat_king = {"id": 22, "z": 0}  # Król Szczurów
#w2 = {"id": 2, "z": 0}         # Ściana piwnicy

# Nowe, hybrydowe definicje portali (ściana + sprajt na jednym polu)
# Używamy ściany w2 (kamiennej), by lepiej pasowała
#st_up =   [w2, {"id": 40, "target": 1}]      # Portal w górę na piętro 1
#st_down = [w2, {"id": 41, "target": -1}]     # Portal w dół do piwnicy -1
#portal_main = [w2, {"id": 41, "target": 0}]      # Portal powrotny na parter

# --- Szablony Kafelków (jedna, poprawna wersja) ---
w = {"id": 1, "z": 0}          # Ściana
s1 = {"id": 5, "z": 0}         # Drzewo
rat = {"id": 3, "z": 0}        # Szczur
merchant = {"id": 20, "z": 0}  # Handlarz
...
st_up = {"id": 40, "target": 1}
st_down = {"id": 41, "target": -1}

WORLD_MAP_0 = [
    [[w],[w],[w],[w],[w],[w],[w],[w],[w],[w]],
    [[w],[],[],[],[],[],[],[],[],[w]],
    [[w],[st_up],[],[merchant],[],[],[healer],[],[],[st_down]],
    [[w],[],[],[],[],[],[],[],[],[w]],
    [[w],[],[],[],[],[],[],[],[],[w]],
    [[w],[],[],[s1],[],[],[],[],[],[w]],
    [[w],[],[],[],[s1],[],[],[],[],[w]],
...
WORLD_MAP_1 = [
    ...
    [[],[w,st_down],[],[],[],[],[],[],[],[w]],
    [[w],[w],[],[],[],[],[],[],[],[w]],
    [[w],[],[],[],[w,{"id":1,"z":0},{"id":2,"z":1},{"id":1,"z":2}],[deer],[],[],[],[w]],
    [[w],[],[],[],[],[],[],[],[],[w]],
    [[w],[],[],[],[],[],[rat],[],[],[w]],
    [[w],[],[],[],[],[],[],[rat_king],[],[w]],
    [[w],[],[],[],[],[],[],[],[],[w]],
...
# --- Mapy Świata (w nowym, spójnym formacie) ---

# Piętro 0: Miasto
#WORLD_MAP_0 = [
#    [[w],[w],[w],[w],[w],[w],[w],[w],[w],[w]],
#    [[w],[],[],[],[],[],[],[],[],[w]],
#    [[w],[],[{"id": 40, "target": 1}],[],[],[],[healer],[],[],[w]],  # Sprajt schodów w górę
#    [[w],[],[],[],[],[],[],[],[],[w]],
#    [[w],[],[],[],[],[],[],[{"id": 41, "target": -1}],[],[w]], # Sprajt schodów w dół
#    [[w],[],[],[],[],[],[],[],[],[w]],
#    [[w],[],[],[merchant],[],[],[],[],[],[w]],
#    [[w],[],[],[],[],[],[],[],[],[w]],
#    [[w],[],[],[s1],[],[],[],[],[],[w]],
...
#    [[w],[w],[w],[w],[w],[w],[w],[w],[w],[w]],
#    [[w],[],[],[],[],[],[],[],[],[w]],
#    [[w],[{"id": 41, "target": 0}],[],[],[],[],[],[],[],[w]], # Sprajt schodów w dół
#    [[w],[],[],[],[],[],[],[],[],[w]],
...
#    [[],[],[],[w2],[rat],[rat],[rat],[w2],[],[]],
#    [[],[],[],[w2],[w2],[w2],[w2],[w2],[],[]],
#    [[],[],[],[],[{"id": 40, "target": 0}],[],[],[],[],[]], # Sprajt schodów w górę
#    [[],[],[],[],[],[],[],[],[],[]],
#    [[],[],[],[],[],[],[],[],[],[]],
#    [[],[],[],[],[],[],[],[],[],[]],
#]
...
class GameState:
    def __init__(self):
        ...
        self.MONSTER_ATTACK_DELAY = 1000
...
# KROK 2: Zastąp swoją starą funkcję main() tymi dwiema funkcjami
### NOWOŚĆ: Logika pętli gry wydzielona do osobnej funkcji ###
def game_loop_step(player, game_state, renderer, sprites, screen, clock, font, ui_font, info_font):
    ...
    if game_state.current_state == 'playing':
        player.update()
    elif game_state.current_state == 'combat' and not game_state.player_turn:
    ...
    if game_state.current_state == 'playing':
        #draw_minimap(screen, player, MAPS); draw_buttons(screen)
...
### =============================================================== ###
###          TESTOWA WERSJA FUNKCJI main() - PODMIEŃ CAŁOŚĆ          ###
### =============================================================== ###
    ...
    for fl, wm in MAPS.items():
        for ry, row in enumerate(wm):
            for rx, vals in enumerate(row):
                ...
                # Używamy kopii listy, aby bezpiecznie z niej usuwać
                for block_data in list(vals): 
                    if block_data['id'] in SPRITE_PROPERTIES:
                        p = SPRITE_PROPERTIES[block_data['id']]
                        target = block_data.get('target', None)
                        
                        sprites.append(Sprite((rx + 0.5) * TILE, (ry + 0.5) * TILE, fl, p, SPRITE_TX[block_data['id']], target_floor=target))
                        
                        # Usuń definicję sprajta z mapy, zostawiając ścianę w tle
                        vals.remove(block_data)
...
class Player:
    def __init__(self, maps):
        ...
        #self.absolute_height = self.floor * TILE + self.height_in_level
        self.pitch = 0
        self.dir_idx = 0
        self.angle = 0.0
        ...
        ### ZMIANA: Rozbudowa ekwipunku i dodanie listy questów ###
        self.equipment = {"weapon": None, "armor": None, "helmet": None, "shield": None}
        self.active_quests = {} # Słownik questów, klucz to nazwa
    ...
    ### POPRAWKA: Uproszczona i poprawna logika zmiany pięter ###

    def change_floor(self, destination_floor):
        if destination_floor in self.maps:
            self.floor = destination_floor
            # Nie musimy już zmieniać self.absolute_height, bo jest @property
    ...
    def grid_move(self, forward, sprites, game_state):
        ...
        # Logika kolizji ze sprajtami, teraz sprawdza atrybut instancji
        for spr in sprites:
            if spr.floor == self.floor and spr.blocking and not spr.is_dead:
                si, sj = int(spr.y // TILE), int(spr.x // TILE)
                if (si, sj) == (i, j):
                    # 1. Sprawdź, czy sprajt ma zdefiniowany cel
                    if spr.target_floor is not None:
                        self.change_floor(spr.target_floor)
                        return # Przenieś gracza i zatrzymaj dalszy ruch

                    # 2. Jeśli nie, sprawdź, czy to potwór
                    if spr.type == 'monster':
                        game_state.start_combat(spr)
                        return # Rozpocznij walkę i zatrzymaj ruch
                    
                    # 3. Jeśli to inny blokujący sprajt, po prostu zablokuj ruch
                    return
    ...
    ### NOWOŚĆ: Logika zarządzania przedmiotami (użyj, załóż) ###
    ...
    ### NOWOŚĆ: Logika zdejmowania przedmiotów ###
...
class Sprite:
    def __init__(self, x, y, floor, properties, texture, target_floor=None):
        ...
        ### NOWOŚĆ: Cel portalu jest teraz atrybutem instancji, a nie definicji ###
...
def draw_player_stats(scr, player, font):
    stats_y, stats_bg_rect = SCREEN_HEIGHT - 60, pygame.Rect(0, SCREEN_HEIGHT - 70, SCREEN_WIDTH, 70); pygame.draw.rect(scr, (0, 0, 0, 150), stats_bg_rect)
