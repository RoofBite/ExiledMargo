...
# Wersja legendy oparta na Twoich oryginalnych nazwach zmiennych

KEY_LEGEND = {
    # --- Ściany ---
    'w': {'type': 'wall', 'id': 1},           # Twoja zmienna 'w'
    'w2': {'type': 'wall', 'id': 2},          # Twoja zmienna 'w2'

    # --- Dekoracje ---
    's1': {'type': 'sprite', 'id': 5},      # Zamiast 's1'
    'b1': {'type': 'sprite', 'id': 12},     # Zamiast 'b1'
    'b2': {'type': 'sprite', 'id': 13},     # Zamiast 'b2'
    'stone': {'type': 'sprite', 'id': 201},    # Twoja zmienna 'stone'
    'barrel': {'type': 'sprite', 'id': 23},   # Zamiast 'barell'

    # --- Postacie niezależne (NPC) ---
    'merchant': {'type': 'sprite', 'id': 20}, # Twoja zmienna 'merchant'
    'healer': {'type': 'sprite', 'id': 21},   # Twoja zmienna 'healer'

    # --- Potwory ---
    'rat': {'type': 'sprite', 'id': 3},       # Twoja zmienna 'rat'
    'deer': {'type': 'sprite', 'id': 14},      # Twoja zmienna 'deer'
    'bear': {'type': 'sprite', 'id': 15},      # Twoja zmienna 'bear'
    'rat_king': {'type': 'sprite', 'id': 22},  # Twoja zmienna 'rat_king'

    # --- Portale i schody ---
    'st_up': {'type': 'portal', 'id': 40},     # Schody w górę (na piętro 1)
    "st_down_m1": {'type': 'portal', 'id': 41},    # Schody w dół (do piwnicy)
    'st_up_0': {'type': 'portal', 'id': 42},     # Schody w górę (na parter)
    'portal_to_las': {'type': 'portal', 'id': 43},   # Portal do lasu (z piętra 0 na 2)
    "st_down_0": {"id": 11, "z": 0, "target": 0},

    # --- Przedmioty do podniesienia ---
    'sztylet': {'type': 'pickup_item', 'id': 101},
    'pink_shell': {'type': 'pickup_item', 'id': 102}, # Twoja zmienna 'pink_shell'

    # --- Klucz do losowego spawnowania ---
    'spawn_rat': {'type': 'spawn', 'id': 3},   # Bardziej opisowa nazwa dla '?'
}
...
def load_map_from_csv(file_name):
    # Upewnij się, że plik jest szukany w tym samym folderze co skrypt
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir,"assets/maps/", file_name)
    with open(file_path, 'r', newline='') as f:
        # Wczytaj dane, zachowując puste komórki jako puste ciągi znaków
...
import json # Upewnij się, że ta linia jest na samej górze pliku

def process_map_cell2(cell):
    """
    Przetwarza pojedynczą komórkę mapy, parsując stringi JSON do obiektów Pythonowych.
    """
    if not isinstance(cell, str) or not cell.strip():
        return cell # Zwróć bez zmian, jeśli komórka jest pusta lub nie jest stringiem

    cell = cell.strip()
    # Sprawdź, czy komórka wygląda jak obiekt JSON
    if (cell.startswith('{') and cell.endswith('}')) or \
       (cell.startswith('[') and cell.endswith(']')):
        try:
            data = json.loads(cell)
            # Logika gry oczekuje listy obiektów w komórce,
            # więc jeśli mamy jeden obiekt (słownik), opakowujemy go w listę.
            if isinstance(data, dict):
                return [data]
            return data
        except json.JSONDecodeError:
            # Jeśli coś pójdzie nie tak z parsowaniem, zwróć oryginalną wartość
            return cell
    return cell
...
def process_map_cell(cell):
    ...
    - JSON dict              -> [dict]
    - JSON list              -> that list
    - text starting with "p1_rat" -> "p1_ratX" (string)
    - inne niepuste teksty   -> ["tekst"]
    """
    # 1) już nie-lista/dict? jeśli nie, zostawiamy
    ...
    # 3) p1_rat.* -> string
    if s.startswith("p1_rat"):
        return s

    # 4) wszystko inne -> jednopozycyjna lista
    return [s]




# Przetworzenie każdej mapy za pomocą nowej funkcji
WORLD_MAP_0 = [list(map(process_map_cell, row)) for row in WORLD_MAP_0]
WORLD_MAP_1 = [list(map(process_map_cell, row)) for row in WORLD_MAP_1]
WORLD_MAP_LAS = [list(map(process_map_cell, row)) for row in WORLD_MAP_LAS]
WORLD_MAP_MINUS_1 = [list(map(process_map_cell, row)) for row in WORLD_MAP_MINUS_1]
...
def load_maps_from_csv(sprite_properties_dict, sprite_textures_dict, key_legend_dict):
    """
    Wczytuje mapy z plików .csv, znajduje też punkty spawnu.
    """
    TILE = 60
    maps = {}
    all_sprites = []
    spawn_points = {}
    map_files = {0: 'assets/maps/world_map_0.csv', 1: 'assets/maps/world_map_1.csv', -1: 'assets/maps/world_map_minus_1.csv', 2: 'assets/maps/world_map_las.csv'}

    for floor_index, filename in map_files.items():
        world_map = []
        spawn_points[floor_index] = []
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                reader = csv.reader(f)
                csv_data = list(reader)
        except FileNotFoundError:
            print(f"Ostrzeżenie: Brak pliku mapy '{filename}'.")
            continue

        for y, row_data in enumerate(csv_data):
            row = []
            for x, key in enumerate(row_data):
                key = key.strip()
                map_cell_content = []

                if key and key in key_legend_dict:
                    obj_info = key_legend_dict[key]
                    obj_type = obj_info.get("type")
                    obj_id = obj_info.get("id")

                    if obj_type == "wall":
                        map_cell_content.append({"id": obj_id, "z": 0})
                    elif obj_type == "spawn":
                        spawn_points[floor_index].append((x, y, obj_id))
                    else:
                        if obj_id in sprite_properties_dict:
                            properties = sprite_properties_dict[obj_id].copy()
                            texture = sprite_textures_dict.get(obj_id)
                            if texture:
                                all_sprites.append(Sprite(
                                    (x + 0.5) * TILE, (y + 0.5) * TILE, floor_index, properties, texture
                                ))

                row.append(map_cell_content)
            world_map.append(row)
        maps[floor_index] = world_map

    return maps, all_sprites, spawn_points
